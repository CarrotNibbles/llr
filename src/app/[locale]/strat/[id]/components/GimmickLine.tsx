    ...
    return (
      <div className={cn("flex text-md", className)}>
        {superMergedGimmicks
          ...
          .map((superMergedGimmick, idx, array) => (
            <div
              key={superMergedGimmick.id}
              className={cn(
                GIMMICK_TEXT_STYLE[superMergedGimmick.type],
                "mr-1"
              )}
            >
...
> & {
  ...
  damageDisplayGimmick?: ArrayElement<
    Exclude<StrategyDataType["raids"], null>["gimmicks"]
  >;
  mergedGimmicks: MergedGimmick[];
  resizePanelSize: number;
            ...
            <HoverCardContent>
              {mergedGimmicks.length > 0 &&
                mergedGimmicks.map((mergedGimmick, index) => (
                  <div
                    key={mergedGimmick.id}
                    className={cn(className, "space-y-1 mb-1")}
                  >
                    <div
                      className={cn(
                        GIMMICK_TEXT_STYLE[mergedGimmick.type],
                        "text-xs",
                        "font-bold"
                      )}
                    >
